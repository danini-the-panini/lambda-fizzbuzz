...
assert(map(to_integer, to_array(list)) == [1, 2, 3])

assert(map(to_integer, to_array(RANGE(ONE)(FIVE))) == [1, 2, 3, 4, 5])
assert(to_integer(FOLD(RANGE(ONE)(FIVE))(ZERO)(ADD)) == 15)
assert(to_integer(FOLD(RANGE(ONE)(FIVE))(ONE)(MULTIPLY)) == 120)
assert(map(to_integer, to_array(MAP(RANGE(ONE)(FIVE))(INCREMENT))) == [2, 3, 4, 5, 6])

assert(to_char(ZED) == 'z')
assert(to_string(FIZZBUZZ) == "FizzBuzz")

assert(map(to_integer, to_array(TO_DIGITS(FIVE))) == [5])
assert(map(to_integer, to_array(TO_DIGITS(POWER(FIVE)(THREE)))) == [1, 2, 5])
