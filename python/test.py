...
assert(not to_boolean(IS_LESS_OR_EQUAL(FIVE)(THREE)))
...
assert(not to_boolean(IS_EMPTY(list)))
assert(to_boolean(IS_EMPTY(EMPTY)))
assert([to_integer(x) for x in to_array(list)] == [1, 2, 3])

assert([to_integer(x) for x in to_array(RANGE(ONE)(FIVE))] == [1, 2, 3, 4, 5])
assert(to_integer(FOLD(RANGE(ONE)(FIVE))(ZERO)(ADD)) == 15)
assert(to_integer(FOLD(RANGE(ONE)(FIVE))(ONE)(MULTIPLY)) == 120)
assert([to_integer(x) for x in to_array(MAP(RANGE(ONE)(FIVE))(INCREMENT))] == [2, 3, 4, 5, 6])

assert(to_char(ZED) == 'z')
assert(to_string(FIZZBUZZ) == 'FizzBuzz')

assert([to_integer(x) for x in to_array(TO_DIGITS(FIVE))] == [5])
assert([to_integer(x) for x in to_array(TO_DIGITS(POWER(FIVE)(THREE)))] == [1, 2, 5])
